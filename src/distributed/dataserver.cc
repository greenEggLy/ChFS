...
// {Your code here}
    auto DataServer::read_data(block_id_t block_id, usize offset, usize len,
        ...
        auto *real_version = (version_t *) (buffer.data() + version_block_offset * (sizeof(version_t) / sizeof(u8)));
...
// {Your code here}
    auto DataServer::alloc_block() -> std::pair<block_id_t, version_t> {
        ...
        auto res1 = block_allocator_->bm->read_block(version_block_id, buffer.data());
        if (res1.is_err()) return {};
        auto *version = (version_t *) (buffer.data() + version_block_offset * (sizeof(version_t) / sizeof(u8)));
        *version += 1;
        auto res2 = block_allocator_->bm->write_block(version_block_id, buffer.data());
