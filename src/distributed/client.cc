...
// {Your code here}
    ...
    -> ChfsResult<std::pair<InodeType, FileAttr>> {
        ...
        auto [type, size, atime, mtime, ctime] = res.unwrap()->as<std::tuple<u64, u64, u64, u64, u8>>();
...
// {Your code here}
    ...
    -> ChfsResult<std::vector<u8>> {
        ...
        while (size > 0) {
            ...
            auto res1 = data_servers_[macId]->call("read_data", blockId, block_info_offset, read_size, version);
    ...
    ChfsNullResult get_or_alloc_block_info(const std::shared_ptr<RpcClient> &metadata_server, inode_id_t id,
...
// {Your code here}
    ...
    -> ChfsNullResult {
        ...
        size_t write_start = 0, write_end = write_len; // pointer to the data of start and end
        // if we should alloc new blocks
        while (write_len > 0) { // still needs to write
            auto left_size_in_block = DiskBlockSize - block_info_offset;
            write_end = left_size_in_block > write_len ? write_start + write_len : write_start + left_size_in_block;
            std::vector<u8> tmp_data(write_end - write_start);
            std::move(data.begin() + (long) write_start, data.begin() + (long) write_end, tmp_data.begin());
            get_or_alloc_block_info(metadata_server_, id, block_info, block_info_index);
            auto [blockId, macId, version] = block_info.at(block_info_index);
            auto res1 = data_servers_[macId]->call("write_data", blockId, block_info_offset, std::move(tmp_data));
            if (res1.is_err()) return res1.unwrap_error();
