...
std::vector<RaftNodeConfig> generate_configs(int num) {
  ...
  for (int i = 0; i < num; i++) {
    RaftNodeConfig config = {.node_id = i,
...
public:
  ...
  void InitNodes(int node_num) {
    ...
    for (auto config : configs) {
      nodes.push_back(std::make_unique<RaftNode<ListStateMachine, ListCommand>>(
          config.node_id, configs));
    ...
    for (auto&& client : clients) {
      while (client->get_connection_state() !=
  ...
  void CheckNoLeader() {
    ...
    for (int i = 0; i < num_nodes; i++) {
      ...
      EXPECT_EQ(std::get<0>(flag_term), false)
          << "Node " << i << " is leader, which is unexpected";
    }
  }
    ...
    for (size_t i = 0; i < configs.size(); i++) {
      ...
      if (has_log) {
        ...
        if (cnt == 1) {
          ...
          EXPECT_EQ(old_value, log_value)
              << "node " << i << " has inconsistent log value: (" << log_value
  ...
  int AppendNewCommand(int value, int expected_nodes) {
    ...
    while (std::chrono::system_clock::now() < end) {
      ...
      for (size_t i = 0; i < configs.size(); i++) {
        ...
        auto res2 = clients[leader_idx]->call(
            RAFT_RPC_NEW_COMMEND, cmd.serialize(cmd.size()), cmd.size());
        if (!res2.is_ok()) {
          continue;
        }
  ...
  int WaitCommit(int index, int num_committed_server, int start_term) {
    ...
    for (int iters = 0; iters < retry_time * 3; iters++) {
      ...
      if (start_term > -1) {
        for (auto&& client : clients) {
          ...
          int current_term =
              std::get<1>(res.unwrap()->as<std::tuple<bool, int>>());
    ...
    int nc = NumCommitted(index);
    EXPECT_GE(nc, num_committed_server)
  ...
  void Restart(int node_id) {
    ...
    nodes[node_id] = std::make_unique<RaftNode<ListStateMachine, ListCommand>>(
        node_id, configs);
    clients[node_id] = std::make_unique<RpcClient>(configs[node_id].ip_address,
                                                   configs[node_id].port, true);
    states[node_id] = std::make_unique<ListStateMachine>();

    while (clients[node_id]->get_connection_state() !=
           rpc::client::connection_state::connected) {
      sleep(1);
    }
